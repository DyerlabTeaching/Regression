...
<mark>In that some of the underlying variation goes towards explaining the patterns in the data and the rest of the variation is *residual* (or left over).</mark>  For regression analyses, consider the simple linear regression model.
...
Where the terms $\beta_0$ is where the expected line intersects the y-axis when $x = 0$, the coefficient $\beta_1$ is the rate at which the $y$ (the results) changes *per unit change* in $x$ (the predictor), and $\epsilon$ is the left over variation (residual) that each point has.
...
The notion here is to estimate the underlying formula for that red line that describes the variation in the original values in how `y` changes systematically across measured values of `x`.


## Least Squares Fitting

So how do we figure this out?  One of the most common ways is to use a method called *Least Squared Distance* fitting.  To describe this, consider a set of hypothetical random models with random values estimated for both the intercept ($\beta_0$) and slope ($\beta_1$) coefficients.  These *could be* close to a good models or not.
...
The "best model" here is one that minimizes the sum of squared distances.
...
First, here is a function that returns the distances between the original points and a hypothesized regression line defined by an intercept and slope from the original points.

```{r}
model_distance <- function( intercept, slope, X, Y ) {
...
In addition to a random search, we can be a bit more systematic about it and make a grid of intercept and slope values, using a *grid search*.
...
You could imagine that we could iteratively zoom in this grid and find the best fit combination of $\beta_0$ and $\beta_1$ values until we converged on a really well fit set.
...
We can see that for the values of the coefficients (labeled *Intercept* and *x*), it has a `model_distance()` of
...
We can estimate a bunch of different models but before we look to see if it is well behaved.  There are several interesting plots that we can examine from the model object such as:
...
Thus far, we've been able to estimate a model, but is it one that explains a significant amount of variation? To determine this, we use the *analysis of variance table*.
...
- `Pr(>F)` is the probability associated with the value of the $F$-statistic and is dependent upon the degrees of freedom for the model and residuals.
...
Notice that the `p-value` is not in this list...  It is estimable from the `fstatistic` and `df` values and here is a quick function that returns the raw p-value by looking up the area under the curve equal to or greater than the observed `fstatistic` with those degrees of freedom.
...
df.models %>%
  ...
  kable( caption = "Model parameters predicting mean ozone in parts per billion measured in New York during the period of 1 May 1973 - 30 September 1973.",
         digits = 3) %>%
  kable_minimal()
```
...
Now, we are estimating two regression coefficients and an intercept.  For three predictors, this gives us 3 more models.
...
df.models %>%
  ...
  kable( caption = "Model parameters predicting mean ozone in parts per billion measured in New York during the period of 1 May 1973 - 30 September 1973.  Values in green indicate the model with the largest variance explained and those in red indicate models with the lowest probability.",
         escape = FALSE) %>%
...
Here is an example using our small data set.   I'm going to make several models, one of which is the original one and the remaining add one more predictor variable that is made up of a random variable.  We will then look at the $R^2$ of each of these models.
...
This is a well-known situation in statistics.  And in fact, we must be very careful when we are examining the differences between models and attempting to decide which set of models are actually better than other sets of models.  

## Akaike Information Content

To get around this, we have a few tools at our disposal.  The most common approach is to look at the *information content* in each model relative to the amount of predictor variables.  In essence, we must *punish* ourselves for adding more predictors so that we do not all run around and add random data to our models.  The most common one is called Akaike Information Criterion (AIC), and provide a general framework for comparing several models.
...
The prevailing notion is that models that have $\delta AIC < 2.0$ should be considered as almost equally informative, whereas <mark>those whose $\delta AIC > 5.0$ are to be rejected as being informative</mark>.  That $2.0 \le \delta AIC \le 5.0$ range is where it gets a bit fuzzy. 
...
df.random %>%
  select( -P ) %>%
  kable( caption = "Model parameters predicting mean ozone in parts per billion measured in New York during the period of 1 May 1973 - 30 September 1973 with variance explained, AIC, and ∂AIC for alternative models.",
         escape = FALSE,
         row.names = FALSE, 
         digits = 3) %>%
  kable_paper( "striped", full_width = FALSE )
